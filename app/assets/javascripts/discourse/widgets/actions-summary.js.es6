...
createWidget('small-user-list', {
  ...
  html(atts) {
    ...
    if (users) {
      ...
      if (postUrl) {
        description = this.fragment('a', { attributes: { href: Discourse.getURL(postUrl) } }, description);
      }
...
createWidget('action-link', {
  ...
  html(attrs) {
    return this.fragment('a', [attrs.text, '. ']);
  },
...
export default createWidget('actions-summary', {
  ...
  html(attrs) {
    ...
    if (attrs.isDeleted) {
      body.push(this.fragment('div.post-action', [
        iconVDom('trash-o'),
        ' ',
        ...
        ' ',
        relativeAge(new Date(attrs.deleted_at))
      ]));
    }
